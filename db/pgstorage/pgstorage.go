...
func (p *PostgresStorage) GetL1Deposits(ctx context.Context, lastExitRoot, exitRoot []byte, dbTx pgx.Tx) ([]*etherman.Deposit, error) {
	updateDepositsStatusSQLInit := fmt.Sprintf(`Select leaf_type, orig_net, orig_addr, amount, dest_net, dest_addr, deposit_cnt, block_id, network_id, tx_hash, metadata, ready_for_claim FROM sync.deposit%[1]v WHERE deposit_cnt <=
	...
	updateDepositsStatusSQL := fmt.Sprintf(`Select leaf_type, orig_net, orig_addr, amount, dest_net, dest_addr, deposit_cnt, block_id, network_id, tx_hash, metadata, ready_for_claim FROM sync.deposit%[1]v WHERE deposit_cnt >
			(SELECT deposit_cnt FROM mt.root%[1]v WHERE root = $1 AND network = 0) AND deposit_cnt <=
			(SELECT deposit_cnt FROM mt.root%[1]v WHERE root = $2 AND network = 0)
			AND network_id = 0 AND ready_for_claim = false`, p.tableSuffix)
	var rows pgx.Rows
	var err error
	if len(lastExitRoot) == 0 {
		rows, err = p.getExecQuerier(dbTx).Query(ctx, updateDepositsStatusSQLInit, exitRoot)
	} else {
		rows, err = p.getExecQuerier(dbTx).Query(ctx, updateDepositsStatusSQL, lastExitRoot, exitRoot)
	}
